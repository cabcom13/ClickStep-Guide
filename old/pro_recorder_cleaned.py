...
class ProEditor(QMainWindow):
    def __init__(self, steps, globals, crop, save_cb, parent_window=None, settings=None):
        super().__init__()
            ...
            if item.item_type == 'blur':
                ...
                try:
                 ...
                 data = {
                     ...
                     'opacity': getattr(item, 'opacity', 0.6),
                     'shape': getattr(item, 'shape', 'rect'),
                     'color': (col.red(), col.green(), col.blue())
                 }
    ...
    def update_properties(self):
        try:
            ...
            if isinstance(item, SpotlightItem):
                ...
                combo.setCurrentText(item.shape)
                combo.currentTextChanged.connect(lambda t: setattr(item, 'shape', t) or item.update())
                self.props_layout.addWidget(combo)
    ...
    def load_step(self, idx):
        ...
        for gl in self.global_layers:
            ...
            if gl.type == 'blur':
                ...
                item.opacity = d.get('opacity', 0.6)
                item.shape = d.get('shape', 'rect')
